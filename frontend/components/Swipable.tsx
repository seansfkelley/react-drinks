...
const InertialSwipable = React.createClass({
  ...
  render() {
    return <div onTouchStart={this._onTouchStart} onTouchMove={this._onTouchMove} onTouchEnd={this._onTouchEnd} className={classnames('inertial-swipable', this.props.className)}>{this.props.children}</div>;
  },
...
const Swipable = React.createClass({
  ...
  _getIndexForDelta(delta) {
    ...
    const shiftedOffsets = _.chain().range(this.state.itemOffsets.length).map(i => this.state.itemOffsets[i] - this.state.itemWidths[i] / 2).value();
    // Why is this -1 again?
  ...
  render() {
    ...
    return <InertialSwipable onSwiping={this._onSwiping} onSwiped={this._onSwiped} itemOffsets={this.state.itemOffsets} initialDelta={this.state.initialDelta} getNearestIndex={this._getNearestIndex} friction={this.props.friction} className={classnames('viewport-container', this.props.className)} ref='inertialSwipable'><div className='sliding-container' ref='slidingContainer' style={{
        WebkitTransform: `translateX(${ offset }px) translateZ(0)`, // Hardware acceleration.
        transform: `translateX(${ offset }px)`
      }}>{this.props.children}</div></InertialSwipable>;
  },
  ...
  _computeCachedState() {
    ...
    const itemOffsets = _.chain(itemWidths).reduce(function (offsets, width) {
