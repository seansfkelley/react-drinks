...
const _recipeListFilter = (listType, ingredientSplits, favoritedRecipeIds) =>
  (() => { switch (listType) {
    ...
    case 'mixable': return recipe => ingredientSplits[recipe.recipeId].missing.length === 0;
    case 'favorites': return recipe => _.contains(favoritedRecipeIds, recipe.recipeId);
    case 'custom': return recipe => !!recipe.isCustom;
  } })()
;

const _sortAndGroupAlphabetical = recipes =>
  _.chain(recipes)
    .sortBy('sortName')
    .groupBy(function(r) {
...
const filteredGroupedRecipes = function({
  ...
  if (baseLiquorFilter == null) { baseLiquorFilter = definitions.ANY_BASE_LIQUOR; }
