...
describe('filteredGroupedIngredients', function () {
  ...
  const GROUPED_INGREDIENTS = [{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }, { searchable: ['a', '2'] }]
  }, {
    name: 'B',
    ingredients: [{ searchable: ['b', '1'] }, { searchable: ['b', '2'] }]
  }];
  ...
  it('should return one group with one match when one ingredient matches', () => filteredGroupedIngredients(toObjectArgs(GROUPED_INGREDIENTS, 'alpha')).should.deep.equal([{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }]
  }]));

  it('should return multiple groups if there are matches in multiple groups', () => filteredGroupedIngredients(toObjectArgs(GROUPED_INGREDIENTS, '1')).should.deep.equal([{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }]
  }, {
    name: 'B',
    ingredients: [{ searchable: ['b', '1'] }]
  }]));

  it('should omit entire groups if they have no matching results', () => filteredGroupedIngredients(toObjectArgs(GROUPED_INGREDIENTS, 'a')).should.deep.equal([{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }, { searchable: ['a', '2'] }]
  }]));

  it('should find the search term as a substring when matching', () => filteredGroupedIngredients(toObjectArgs(GROUPED_INGREDIENTS, 'lph')).should.deep.equal([{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }]
  }]));

  return it('should be case-insensitive when matching', () => filteredGroupedIngredients(toObjectArgs(GROUPED_INGREDIENTS, 'ALPHA')).should.deep.equal([{
    name: 'A',
    ingredients: [{ searchable: ['a', '1', 'alpha'] }]
  }]));
});
